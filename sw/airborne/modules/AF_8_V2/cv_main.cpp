 ...
 * Per-node and total callback durations are written each frame into
 * the t_*_us globals below.  Expose them as dl_settings in
 * af8_vision.xml to read them live in the GCS Settings panel over
 * WiFi — no OptiTrack required.
 */
...
/* ── cycle-time profiling (µs) — read via GCS dl_settings ──────── */
volatile uint32_t t_ground_us     = 0;
volatile uint32_t t_obstacle_us   = 0;
volatile uint32_t t_gate_us       = 0;
volatile uint32_t t_bottom_us     = 0;
volatile uint32_t t_controller_us = 0;
volatile uint32_t t_draw_us       = 0;
volatile uint32_t t_total_cb_us   = 0;
                         ...
                         const ControlOutput &ctrl)
{
    ...
    /* ── timing overlay — bottom-right corner ───────────────────── */
    char tbuf[64];
    snprintf(tbuf, sizeof(tbuf), "tot:%lu gnd:%lu obs:%lu",
             (unsigned long)t_total_cb_us,
             (unsigned long)t_ground_us,
             (unsigned long)t_obstacle_us);
    ov_txt(canvas, tbuf, {4, h - 36}, {200, 200, 200}, 0.33);
    snprintf(tbuf, sizeof(tbuf), "gate:%lu bot:%lu ctrl:%lu drw:%lu",
             (unsigned long)t_gate_us,
             (unsigned long)t_bottom_us,
             (unsigned long)t_controller_us,
             (unsigned long)t_draw_us);
    ov_txt(canvas, tbuf, {4, h - 50}, {200, 200, 200}, 0.33);
}
                                  ...
                                  uint8_t camera_id __attribute__((unused)))
{
    ...
    uint32_t t_cb_start = get_sys_time_usec();
    uint32_t t_a;
    ...
    t_a = get_sys_time_usec();
    ground_edge_node_process(rotated.data, rot_w, rot_h, &local.ground_edge);
    t_ground_us = get_sys_time_usec() - t_a;

    t_a = get_sys_time_usec();
    // obstacle_detector_node_process(bgr.data, bgr.cols, bgr.rows, &local.obstacle);
    obstacle_detector_node_process(rotated.data, rotated.cols, rotated.rows, &local.obstacle);
    t_obstacle_us = get_sys_time_usec() - t_a;

    t_a = get_sys_time_usec();
    gate_detector_node_process(rotated.data, rot_w, rot_h, &local.gate);
    t_gate_us = get_sys_time_usec() - t_a;

    t_a = get_sys_time_usec();
    bottom_cam_node_process(bgr.data, bgr.cols, bgr.rows, &local.bottom_cam);
    t_bottom_us = get_sys_time_usec() - t_a;

    local.valid = true;

    /* ── Run controller — timed ─────────────────────────────── */
    t_a = get_sys_time_usec();
    ControlOutput cmd = controller_update(&s_ctrl, &local, rot_w);
    t_controller_us = get_sys_time_usec() - t_a;

    /* ── Publish globals ────────────────────────────────────── */
    *((FrameResults  *)&frame_results) = local;
    *((ControlOutput *)&ctrl_output)   = cmd;

    /* ── Draw overlay and write back into img->buf — timed ───── */
    t_a = get_sys_time_usec();
#if AF_8_V2_DRAW
    /* Annotate the rotated frame */
    draw_overlay(rotated, local, cmd);
    ...
    memcpy(img->buf, yuv_out.data,
           (size_t)img->w * (size_t)img->h * 2u);
#endif
    t_draw_us = get_sys_time_usec() - t_a;

    t_total_cb_us = get_sys_time_usec() - t_cb_start;

    printf("CV t=%lu gnd=%lu obs=%lu gate=%lu bot=%lu ctrl=%lu drw=%lu\n",
           (unsigned long)t_total_cb_us,
           (unsigned long)t_ground_us,
           (unsigned long)t_obstacle_us,
           (unsigned long)t_gate_us,
