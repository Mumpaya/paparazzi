...
static const int   MAX_CORNERS        = 40;  /* Reduced from 150 for 30-40% speedup */
...
/* ── pre-allocated intermediate buffers (avoid per-frame allocations) */
static cv::Mat s_bgr_input;        /* cached input frame */
static cv::Mat s_gray;             /* grayscale conversion */
static cv::Mat s_enhanced;         /* CLAHE enhanced */
static cv::Mat s_rotated;          /* rotated frame */
static cv::Mat s_small;            /* downsampled for Hough */
static cv::Mat s_blurred;          /* blurred for edge detection */
static cv::Mat s_edges;            /* full-size edges output */
static cv::Mat s_roi_hsv;          /* HSV of ROI region */
static cv::Mat s_morph_kernel;     /* morphological kernel */
static cv::Mat s_dilated;          /* dilated edges for panel detection */
static cv::Mat s_dil_kernel;       /* dilation kernel */
                         ...
                         cv::Mat &edges_out)
{
    ...
    /* Downsample to half res — 4x fewer pixels for Canny + Hough
       Use pre-allocated buffers s_small, s_blurred, s_edges */
    cv::resize(roi_frame, s_small, cv::Size(), 0.5, 0.5, cv::INTER_LINEAR);

    cv::GaussianBlur(s_small, s_blurred, cv::Size(HOUGH_BLUR_K, HOUGH_BLUR_K), 0);
    cv::Canny(s_blurred, s_edges, HOUGH_CANNY_LO, HOUGH_CANNY_HI);

    /* Scale edges back to full ROI size for Stage 7 reuse */
    cv::resize(s_edges, edges_out, roi_frame.size(), 0, 0, cv::INTER_NEAREST);

    std::vector<cv::Vec4i> lines;
    cv::HoughLinesP(s_edges, lines, 1.0, CV_PI / 180.0,
                          ...
                          PoleDetection *out_boxes, int *n_boxes)
{
    ...
    /* Reuse pre-allocated buffer (s_roi_hsv will hold mask) */
    cv::Mat &mask = s_roi_hsv;
    cv::inRange(roi_hsv, ORANGE_HSV_LO, ORANGE_HSV_HI, mask);

    /* Reuse pre-allocated morphological kernel */
    cv::morphologyEx(mask, mask, cv::MORPH_OPEN,  s_morph_kernel);
    cv::morphologyEx(mask, mask, cv::MORPH_CLOSE, s_morph_kernel);
                          ...
                          float *ground_obstacle_scores)
{
    ...
    /* Reuse pre-allocated buffer (s_roi_hsv will hold green_mask) */
    cv::Mat &green_mask = s_roi_hsv;
    cv::inRange(roi_hsv, GREEN_HSV_LO, GREEN_HSV_HI, green_mask);

    /* Reuse pre-allocated morphological kernel */
    cv::morphologyEx(green_mask, green_mask, cv::MORPH_OPEN,  s_morph_kernel);
    cv::morphologyEx(green_mask, green_mask, cv::MORPH_CLOSE, s_morph_kernel);
                         ...
                         float *panel_scores)
{
    ...
    /* dilate slightly to close small border gaps
       Use pre-allocated buffers s_dilated and s_dil_kernel */
    cv::dilate(edges, s_dilated, s_dil_kernel, cv::Point(-1,-1), 1);

    std::vector<std::vector<cv::Point>> contours;
    cv::findContours(s_dilated, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
                      ...
                      GapCandidate *out_gaps, int *n_gaps)
{
    ...
    for (auto &r : runs) {
        ...
        float safety = clampf(GAP_W_WIDTH*w_sc + GAP_W_CLEAR*c_sc + GAP_W_CENTER*cent_sc,
                              0.0f, 1.0f);
        GapCandidate gc;
        gc.rank=0; gc.center_x=(int)(gap_centre*col_width);
...
void obstacle_detector_node_init(void)
{
    ...
    /* ── pre-allocated buffers are created lazily in process() ──
       s_bgr_input, s_gray, s_enhanced, s_rotated, etc. get
       allocated to match the actual input frame dimensions.
    */
    s_morph_kernel = cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(3, 3));
    s_dil_kernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3));
}
