...
class camera:
    def __init__(self, pos=np.array([0.0, 0.0]), rot=0.0, fov=90.0):
        ...
        print(res*res)
        arena_radius = 4
        self.memory = np.zeros((res, res))
        self.memory_area_low_limit = np.array([-arena_radius, -arena_radius])
        self.memory_area_high_limit = np.array([arena_radius, arena_radius])
        self.depth_noise_std = 0.0

    def pos_to_memory_coords(self, pos):
        norm_pos = (pos - self.memory_area_low_limit) / (self.memory_area_high_limit - self.memory_area_low_limit)
        return np.clip((norm_pos * np.array(self.memory.shape)).astype(int), 0, np.array(self.memory.shape) - 1)
    ...
    def depth_view(self, objs, num_rays=10):
        ...
        for angle in angles:
            ...
            for obj in objs:
                ...
                depth*=np.random.normal(1,self.depth_noise_std)
                if depth < min_depth:
                    min_depth = depth
    ...
    def record_depths(self, objs, num_rays=10):
        ...
        for angle, depth, dir in zip(angles, depths, dirs):
            ...
            mem_coord = self.pos_to_memory_coords(end_point)
            self.memory[mem_coord[0], mem_coord[1]] += 1
    ...
    def plot_memory(self):
        plt.imshow(self.memory.T, origin='lower',
                   extent=(self.memory_area_low_limit[0], self.memory_area_high_limit[0], self.memory_area_low_limit[1], self.memory_area_high_limit[1]),
                   cmap='gray_r')
        plt.colorbar(label='Memory Value')
        plt.title('Camera Memory Map')
...
def main() -> None:
    ...
    for obj in objs:
        obj.plot()
    cam = camera(pos=np.array([-3.0, 0.0]), rot=0.0)
    cam.plot()
    cam.depth_noise_std = 0.08
    for _ in range(50):
        move_delta = np.random.uniform(-4.0, 4, size=2)
        move_delta[0] = 0
        rot_delta = np.random.uniform(-45, 45)
        cam.pos += move_delta
        cam.rot += rot_delta
        cam.record_depths(objs, 10)
        cam.plot()
        cam.rot -= rot_delta
        cam.pos -= move_delta
    plt.show()
