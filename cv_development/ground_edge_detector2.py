...
class GroundEdgeDetector:
    ...
    def mask_non_uniform(self, bgr: np.ndarray, ksize: int = 25, thresh: float = 15.0) -> np.ndarray:
        """
    ...
    def detect(self, bgr: np.ndarray) -> GroundEdgeResult:
        ...
        contours, _ = cv2.findContours(mask_good, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        contours = [c for c in contours if cv2.contourArea(c) > self.min_area]

        eroded   = cv2.erode(mask_good, np.ones((3, 3), np.uint8), iterations=1)
        boundary = cv2.subtract(mask_good, eroded)
        ...
        for angle, mid, p1, p2 in frame_lines:
            ...
            reason   = self._is_mat_edge(p1, p2, mask_good, hsv, mask_bad)
            if reason:
                rejected_lines.append((ep1, ep2, reason))
            else:
                confirmed_lines.append((ep1, ep2, p1, p2))

        edge = np.zeros_like(mask_good)
    ...
    def draw(self, bgr: np.ndarray, result: GroundEdgeResult) -> np.ndarray:
        ...
        for p1, p2 in result.lines:
            cv2.line(out, p1, p2, (255, 180, 180), 4)
